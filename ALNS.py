...
DATA_PATH = PROJECT_DIR / "data" / "c209.csv"
OUTPUT_DIR = PROJECT_DIR / "results1" / "results"/"Comparison_Test_Batch_01s"
...
def load_c209_csv(path: Path):
    ...
    if 0 not in NODE_MAP:
        raise ValueError("c209.csv 中没有找到编号为 0 的配送中心。")

    customers = sorted([i for i in NODE_MAP.keys() if 1 <= i <= 100])

    if not customers:
        raise ValueError("没有读取到客户节点，请检查 c209.csv 列顺序是否为 id,x,y,demand,ready,due,service。")
...
def evaluate_by_original_cost(chrom, depot, customers, stations, mode=MODE):
    ...
    for cid in chrom:
        ...
        if node_id not in NODE_MAP:
            raise KeyError(f"客户节点 {node_id} 不在 NODE_MAP 中，请检查 c209.csv 是否读取完整。")
...
def main():
    ...
    depot, customers = load_c209_csv(DATA_PATH)
